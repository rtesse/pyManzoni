...
class MenuBar:
    ...
    def create_edit_menu_file(self):
        self.loadAction = QAction(QIcon(), "Load", self.mainMenu)
        self.loadAction.setShortcut("Ctrl+L")

        self.trackAction = QAction(QIcon(), "Track", self.mainMenu)
        self.trackAction.setShortcut("Ctrl+T")

        self.saveAction = QAction(QIcon(), "Save", self.mainMenu)
    ...
    def create_help_menu_file(self):
        ...
        self.helpManzoni = QAction(QIcon(), "Manzoni", self.mainMenu)
        self.helpManzoni.triggered.connect(
            lambda _: self.open_webbrowser("https://ulb-metronu.github.io/georges/"),
        )
        self.helpZgoubi = QAction(QIcon(), "Zgoubi", self.mainMenu)
        self.helpZgoubi.triggered.connect(
            lambda _: self.open_webbrowser(
                "https://sourceforge.net/projects/zgoubi/lists/zgoubi-svn",
            ),
        )
